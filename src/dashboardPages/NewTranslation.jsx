    ...
    <div className="max-w-5xl mx-auto space-y-6 relative">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 w-[90%] max-w-md space-y-6 shadow-xl">
            ...
            <div className="space-y-5">
              ...
              {["Analyzing", "Calculating Pages", "Generating order summary"].map((label, index) => {
                const step = index + 1;
                return (
                  <div key={label} className="flex items-center gap-3">
                    {processStep > step ? (
                      ...
                      <div className={`w-5 h-5 rounded-full border-2 ${processStep === step ? "border-blue-600 animate-spin border-t-transparent" : "border-gray-300"}`} />
                    )}
                    <span className={processStep >= step ? "text-blue-600" : "text-gray-400"}>
                      {label}
                    </span>
                  </div>
                );
